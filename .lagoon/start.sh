...
function configure_lagoon_realm {
    ...
    CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=lagoon-ui --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
...
function configure_opendistro_security_client {
    ...
    if [ "$searchguard_client_id" != "[ ]" ]; then
        ...
        SEARCHGUARD_CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=searchguard --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
    ...
    if [ "$lagoon_searchguard_client_id" != "[ ]" ]; then
        ...
        LAGOON_SEARCHGUARD_CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=lagoon-searchguard --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
    ...
    CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=lagoon-opendistro-security --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
...
function configure_api_client {
    ...
    auth_server_client_id=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients?clientId=auth-server --config $CONFIG_PATH)
    if [ "$auth_server_client_id" != "[ ]" ]; then
        echo "Client auth server is already created, skipping basic setup"
    else
      ...
      AUTH_SERVER_CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=auth-server --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
      REALM_MANAGEMENT_CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=realm-management --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
      ...
      AUTH_SERVER_CLIENT_POLICY_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$REALM_MANAGEMENT_CLIENT_ID/authz/resource-server/policy?name=Client+auth-server+Policy --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
      # 4 Update user impersonate permission to add client policy (PUT)
      IMPERSONATE_PERMISSION_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$REALM_MANAGEMENT_CLIENT_ID/authz/resource-server/permission?name=admin-impersonating.permission.users --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
    ...
    echo "We get here"
        set +e
        lagoon_role=$(/opt/jboss/keycloak/bin/kcadm.sh get roles/${composite_role_names[$crn_key]} -r lagoon --config $CONFIG_PATH)

        if [[ "$lagoon_role" = Resource* ]]; then
          set -e
          ...
          echo "Lagoon ${composite_role_names[$crn_key]} role already exists, skipping"
        fi
    done
    ...
    CLIENT_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon clients?clientId=api --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
    ADMIN_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/admin --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    GUEST_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/guest --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    REPORTER_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/reporter --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    DEVELOPER_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/developer --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    MAINTAINER_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/maintainer --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    OWNER_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/owner --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
    PLATFORM_OWNER_ROLE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get  -r lagoon roles/platform-owner --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)["id"])')
...
EOF
    ...
    DEFAULT_PERMISSION_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$CLIENT_ID/authz/resource-server/permission?name=Default+Permission --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["id"])')
...
function add_group_viewall {
  ...
  GROUP_RESOURCE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$CLIENT_ID/authz/resource-server/resource?name=group --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["_id"])')
...
function add_deployment_cancel {
  ...
  DEPLOYMENT_RESOURCE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$CLIENT_ID/authz/resource-server/resource?name=deployment --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["_id"])')
...
function configure_task_cron {
  ...
  TASK_RESOURCE_ID=$(/opt/jboss/keycloak/bin/kcadm.sh get -r lagoon clients/$CLIENT_ID/authz/resource-server/resource?name=task --config $CONFIG_PATH | python3 -c 'import sys, json; print(json.load(sys.stdin)[0]["_id"])')
