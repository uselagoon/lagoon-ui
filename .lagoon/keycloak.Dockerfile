...
RUN microdnf install -y nc python3 \
    && microdnf clean all \
    && rm -rf /var/cache/yum
...
COPY .lagoon/start.sh /opt/jboss/tools/start.sh
COPY .lagoon/wait-for-mariadb.sh /lagoon/entrypoints/99-wait-for-mariadb.sh
COPY .lagoon/profile.properties /opt/jboss/keycloak/standalone/configuration/profile.properties
