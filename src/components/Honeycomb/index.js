...
const Honeycomb = ({ data, filter }) => {
  ...
  const handleHexClick = project => {
    ...
    setProjectInView({
      ...
      severityCount: { critical: critical, high: high, medium: medium, low: low },
    });
  };
  ...
  const getClassName = critical => {
    ...
    if (critical === 1) {
      return 'light-red';
    } else if (critical >= 1 && critical <= 5) {
      return 'red';
    } else if (critical >= 5 && critical < 10) {
      return 'dark-red';
    } else if (critical >= 10) {
      return 'darker-red';
    }
  ...
  useEffect(() => {
    ...
    if (count >= 49 && count <= 120)
      setDisplay({ type: 'small', multiplier: 3, hexSize: 2, viewBox: '125 -20 100 100' });
    if (count >= 121 && count <= 384)
      setDisplay({ type: 'smaller', multiplier: 4, hexSize: 1, viewBox: '70 -10 100 100' });
              ...
              {hexs.slice(0, projects.length).map((hex, i) => {
                ...
                const HexText = () => {
                  ...
                  if (problemsPerProject.length) {
                    return <Text className={classes}>{`P: ${problemCount}, C: ${critical}`}</Text>;
                  } else {
                ...
                return (
                  <Hexagon
                    ...
                    r={hex.r}
                    s={hex.s}
                    className={getClassName(critical)}
                    onClick={() => handleHexClick(project)}
                  >
                    <HexText />
                  </Hexagon>
                );
                              ...
                              <div className="overview">
                                <ul className="overview-list">
                                  <li className="result">
                                    ...
                                    <span className="text-large">{Object.keys(problems).length}</span>
                                  </li>
                                  <li className="result">
