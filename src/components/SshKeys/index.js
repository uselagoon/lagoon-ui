...
const SshKeys = ({ me: { id, email, sshKeys: keys }, loading, handleRefetch }) => {
  ...
  const [ updateUserSSHPublicKeyMutation, { loading: updateLoading, error: updateError, data: updateData } ] = useMutation(UpdateUserSSHPublicKey, {
    onCompleted: () => {
      handleRefetch();
      closeModal();
    },
  ...
  const [ deleteUserSSHPublicKeyMutation, { called: deleteCalled, error: deleteError } ] = useMutation(DeleteUserSSHPublicKey, {
  ...
  const tooltipDateTime = (created, lastUsed) => {
    return (
        ...
        <span>
          ...
          <br />
          <span>
          <b>Last Used:</b> {lastUsed != null ? moment.utc(lastUsed).local().format('DD MMM YYYY, HH:mm:ss (Z)') : 'Never'}
        </span>
        </>
    );
  };
