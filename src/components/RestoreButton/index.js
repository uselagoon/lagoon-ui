...
const RestoreButton = ({ backup: { backupId, restore } }) => {
  ...
  if (restore.status === 'pending') return <Button disabled>{t('backups.restore.retrieving')}</Button>;

  if (restore.status === 'failed') return <Button disabled>{t('backups.restore.retrieveFailed')}</Button>;
