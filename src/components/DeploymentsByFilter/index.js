...
const DeploymentsByFilter = ({ deployments }) => {
  const {
    ...
    getClassNamesFor,
    requestSort,
  } = useSortableData(deployments, { key: "created", direction: "descending" });
  ...
  const formatString = (textToEdit, labelClassToQuery) => {
    ...
    const labelWidth = document
      .querySelector(`.data-row > .${labelClassToQuery}`)
  ...
  return (
    <Deployments>
      <div className="filters">
        <label>
          {sortedItems.length == 1
            ? `1 deployment`
            : `${sortedItems.length} deployments`}
        </label>
        <label></label>
        <input
          type="text"
      ...
      <DeploymentsDataTable>
        ...
        {!sortedItems.filter((deployment) => filterResults(deployment))
          .length && <div className="data-none">No deployments</div>}
        {sortedItems
            ...
            return (
              <div className="data-row row-heading" key={deployment.id}>
                ...
                <div className="environment">
                  <DeploymentsLink
                    environmentSlug={
                      deployment.environment.openshiftProjectName
                    }
                    projectSlug={deployment.environment.project.name}
                  >
